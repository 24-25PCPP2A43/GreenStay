...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($sujet) || empty($message) || empty($client_id) || empty($reservation_id)) {
        ...
        $fichiers_string = implode(',', $fichiers_paths);

        $sql = "INSERT INTO reclamations (sujet, message, statut, client_id, reservation_id, fichiers)
                VALUES (:sujet, :message, 'Nouveau', :client_id, :reservation_id, :fichiers)";
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':sujet', $sujet);
        $stmt->bindParam(':message', $message);
        $stmt->bindParam(':client_id', $client_id);
        $stmt->bindParam(':reservation_id', $reservation_id);
        $stmt->bindParam(':fichiers', $fichiers_string); // Bind the comma-separated string
...
// Validate and sanitize the $tri parameter to prevent SQL injection
$allowed_tri_values = ['sujet', 'date_reservation'];
if (!in_array($tri, $allowed_tri_values)) {
    $tri = 'sujet'; // Default value
}
...
$sql = "SELECT r.id, r.sujet, r.message AS reclamation_message, r.statut, c.nom, c.prenom, re.date_reservation,
                rep.message AS reponse_message, rep.date_reponse, r.fichiers
        FROM reclamations r
...
<head>
    ...
    <style>
        ...
        .reclamation-section {
            min-height: 100vh;
            background-image: url('../../assets_reclamations/images/best-03.jpg');
...
<body>
    ...
    <div class="container-fluid reclamation-section">
                                ...
                                <?php if (count($result) > 0): ?>
                                    <?php foreach ($result as $row): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php
                                                    if (!empty($row['fichiers'])) {
                                                        $fichiers = explode(',', $row['fichiers']);
                                                        foreach ($fichiers as $fichier):
                                                            if (!empty($fichier)):
                                                                $file_name = basename($fichier);
                                                                $file_ext = strtolower(pathinfo($fichier, PATHINFO_EXTENSION));
                                                                ?>
                                                                <a href="<?= htmlspecialchars($fichier) ?>" target="_blank"><?= htmlspecialchars($file_name) ?></a><br>
                                                                <?php
                                                            endif;
                                                        endforeach;
                                                    } else {
   ...
   data-id="<?= $row['id'] ?>">Supprimer</a>
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="9" class="text-center text-muted">Aucune réclamation trouvée.</td>
                                    </tr>
                                <?php endif; ?>
                                </tbody>
                            </table>
                        ...
                        <div class="text-center mt-2">
                            <a href="home.php" class="btn" style="background-color: #ADD8E6; color: white; padding: 10px 20px;">Retour à l'Accueil</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('sendBtn').addEventListener('click', () => {
            ...
            let response = "Désolé, je ne comprends pas. Pouvez-vous reformuler ?";

            if (userInput.toLowerCase().includes("gestion service") || userInput.toLowerCase().includes("problème avec le service")) {
                response = "Nous sommes désolés d'apprendre que vous rencontrez un problème avec notre service. Un responsable de service vous contactera dans les plus brefs délais pour résoudre votre problème.";
            }
            else if (userInput.toLowerCase().includes("réclamation")) {
                response = "Pour soumettre une réclamation, veuillez remplir le formulaire fourni.";
            } else if (userInput.toLowerCase().includes("statut")) {
                response = "Vous pouvez vérifier le statut de votre réclamation dans votre espace client.";
            }
            // Add new question/answer pairs here:
            else if (userInput.toLowerCase().includes("annuler réservation") || userInput.toLowerCase().includes("annulation réservation")) {
                response = "Pour annuler votre réservation, veuillez contacter notre service clientèle au [+21629456230] ou par email à [grrenstay@gmail33.com].";
            }
            else if (userInput.toLowerCase().includes("modifier réservation") || userInput.toLowerCase().includes("changer réservation")) {
                response = "Pour modifier votre réservation, veuillez vous connecter à votre compte sur notre site web ou contacter notre service clientèle.";
            }
            else if (userInput.toLowerCase().includes("horaires d'ouverture") || userInput.toLowerCase().includes("quand êtes-vous ouvert")) {
                response = "Notre service clientèle est disponible du lundi au vendredi de 9h à 18h.";
            }
            // End of added questions

            document.getElementById('chatbox').innerHTML += `<div style="color: black;"><strong>Assistant:</strong> ${response}</div>`;
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', function (e) {
            ...
            fetch(`supprimerReclamation.php?id=${id}`, {
                method: 'GET'
            })
