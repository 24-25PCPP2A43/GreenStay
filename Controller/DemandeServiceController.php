...
class DemandeServiceController {
    ...
    public function submitDemande() {
        // Démarrer la session pour stocker les notifications
        session_start();

        // Vérifier si le formulaire a été soumis (par exemple, via POST)
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
            // Logique pour enregistrer la demande (par exemple, insérer dans la base de données)
            // Supposons que la demande a été enregistrée avec succès

            // Ajout d'une notification en session
            if (!isset($_SESSION['notifications'])) {
                $_SESSION['notifications'] = [];
            }

            $_SESSION['notifications'][] = "Nouvelle demande reçue.";
        }

        // Redirection ou rendu de la vue après traitement (selon la logique de ton application)
        header('Location: back_office.php?page=demandes');
        exit;
    }
    ...
    // L'id_utilisateur EST l'id du client
    $clientId = $_SESSION['user']['id_utilisateur'];
